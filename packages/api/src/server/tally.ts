...
import { DecryptionShareResponse, CiphertextTally, CiphertextTallyDecryptionShare, CiphertextTallyQueryResponse, DecryptionShareRequest, DecryptTallyRequest, DecryptTallyShareRequest, PlaintextTally, PlaintextTallyQueryResponse } from '../models/tally';
import { get, post } from '../utils/http';



export const decryptShareTally = async (election_id: string, tally_name: string): Promise<DecryptionShareResponse | undefined> => {

    const path = `${process.env.REACT_APP_GUARDIAN_SERVICE}tally/decrypt-share?election_id=${election_id}&tally_name=${tally_name}`;
    const response = await get<{ status:string, message:string, shares:DecryptionShareResponse }>(path);
    return response.parsedBody?.shares;
}



export const decryptSharePostTally = async (guardian_id: string, encrypted_tally: CiphertextTally, context: CiphertextElectionContext): Promise<DecryptionShareResponse | undefined> => {
    ...
    const response = await post<{ status:string, message:string, shares:DecryptionShareResponse }>(path, data);
    return response.parsedBody?.shares;
}


export const getTallyDecrypt = async (election_id: string, tally_name: string, guardian_id: string): Promise<CiphertextTallyDecryptionShare[] | undefined> => {
...
export const submitShareTally = async (share: CiphertextTallyDecryptionShare): Promise<boolean | undefined> => {
...
export const findTallyDecrypt = async (tally_name: string, skip: number, limit:number): Promise<CiphertextTallyDecryptionShare[] | undefined> => {
...
export const getTally = async (election_id: string, tally_name: string): Promise<CiphertextTally | undefined> => {
...
export const postTally = async (election_id: string, tally_name: string): Promise<CiphertextTally | undefined> => {
    const path = `${process.env.REACT_APP_MEDIATOR_SERVICE}tally?election_id=${election_id}&tally_name=${tally_name}`;
    const response = await post<{ resp: CiphertextTally }>(path, {});
    return response.parsedBody?.resp;
}

export const findTally = async (election_id: string, skip: number, limit: number): Promise<CiphertextTally[] | undefined> => {
    const data: BaseQueryRequest = {
        filter: {}
    };
...
export const decryptTally = async (election_id: string, tally_name: string, restart: boolean): Promise<PlaintextTally[] | undefined> => {
